...
struct AppConfig {
  ...
  // ---- Persist-история во флеш (переживает перезагрузку/перепрошивку) ----
  // В отличие от PSRAM-буфера (пишется на каждое чтение), сюда пишется
  // периодический снимок ("checkpoint") всех регистров разом -- частая
  // запись каждого отдельного чтения убила бы флеш по циклам стирания.
  bool flash_history_enabled = false; // выключено по умолчанию -- осознанный выбор, не всем нужно
  int flash_history_interval_min = 5;
